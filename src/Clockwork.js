...
function Clockwork() {
	...
	m.unpackCaliber=function(clbrname, toppack, xeltoss){
		var i,clbr=ProtoSS.getDefinitionByName(clbrname),
			clbrcls,xcls=[],
			cls=clbr.classes,sets=clbr.settings||{},types=clbr.types||{},args=clbr.args||{};
	...
	m.caliberClass=function(name, supers, settings, defname, superargs, supertype){
		var cfn=function(cdata){
			...
			m._calibrate=function(){
				var s=o.constructor.caliberSettings,k;
				for(k in s)o[k]=s[k];
			};
			m._calibrate.call(o);
		...
		cfn.aname=name;
		if(defname)Object.defineProperty(cfn, "name", {value:name});
		return cfn;
	};
