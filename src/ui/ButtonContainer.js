	...
	m.configButtonShape=function(btnsh, listeners, interaction, mouseEnabled, mouseChildren){
		...
		if(interaction){
			if(o.mousec && o.mousec.interaction){
				o.mousec.interaction.setInteractID(btnsh, {mouseEnabled:mouseEnabled,mouseChildren:mouseChildren});
			} else if(o.autoInteraction){
				if(!o.interactionQueue)o.interactionQueue=[];
				o.interactionQueue.push({data:btnsh,mouseEnabled:mouseEnabled,mouseChildren:mouseChildren});
			}
		}
		return o;
	};
...
ProtoSS.staticProps(zetaret.global.packages.clockwork.ui.ButtonContainerStatis,{statis:{drag:true}});
