...
function Button() {
	...
	m.setInteraction = function(mousec) {
		...
		if (mousec && mousec.interaction)
			mousec.interaction.setInteractID(o, {
				mouseEnabled: false,
				mouseChildren: true
			});
		return o;
	...
	m.doInteractionQueue = function() {
		var iq = o.interactionQueue,
			l = iq ? iq.length : 0,
			i;
		if (l > 0 && o.mousec && o.mousec.interaction) {
			for (i = 0; i < l; i++) {
				o.mousec.interaction.setInteractID(iq[i].data, {
					mouseEnabled: iq[i].mouseEnabled,
					mouseChildren: iq[i].mouseChildren
				});
			}
			o.interactionQueue = null;
		}
	...
	m.configButton = function(btn) {
		...
		if (o.mousec && o.mousec.interaction) {
			o.mousec.interaction.setInteractID(btn, {
				mouseEnabled: false,
				mouseChildren: true
			});
...
ProtoSS.staticProps(zetaret.global.packages.clockwork.ui.Button, {
SELECTED: "selected",
