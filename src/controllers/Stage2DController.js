...
function Stage2DController() {
	...
	m.precise16 = function(n) {
		return Math.round(n * 10000000000000000) / 10000000000000000;
	};
	m.precise15 = function(n) {
		return Math.round(n * 1000000000000000) / 1000000000000000;
	};
	m.precise14 = function(n) {
		return Math.round(n * 10000000000000) / 10000000000000;
	};
	m.precise12 = function(n) {
		return Math.round(n * 1000000000000) / 1000000000000;
	};
	m.precise10 = function(n) {
		return Math.round(n * 10000000000) / 10000000000;
	};
	m.precise8 = function(n) {
		return Math.round(n * 100000000) / 100000000;
	};
	m.precise6 = function(n) {
		return Math.round(n * 1000000) / 1000000;
	};
	m.precise4 = function(n) {
		return Math.round(n * 10000) / 10000;
	};
	m.precise2 = function(n) {
	...
	m.invalidateStageChild = function(child, f, d, parent, list, mode, childo) {
		if (child.invalidateTransform) {
			if (!child.forceSelfInvalidate) {
				var cos1, sin1, cos2, sin2, rx = child.rx,
					ry = child.ry,
					sx = child.sx,
					sy = child.sy,
					t = child.transform;
	...
	m.renderStageChild = function(child, f, d, parent, list, mode, childo) {
		...
		try {
			...
			if (gr || child.canrender || child.mask || child.cacheLevel || child.layer || child.cachetransforms) {
				var ot = o.t,
					oct = o.ct,
					t, ct, dtop, a, zt, zct, rendy = o.renderer,
					ctx = rendy.context,
					rm = o.rendererMediator,
					ctxc, cb, dlt = o.defaultLocalTransform,
					allowDraw = true,
					s = rendy.setters,
					clearReset = false,
					exith, prevblend;
				...
				if (child.cacheLevel) {
					...
					if (!ctx) ctx = o.createContext(child.cacheLevel, child.opaqueBackground ? {
						alpha: false
					} : child.cacheLevelData);
	...
	m.renderStage = function(data, parent) {
		...
		if (o.prebufferlist) {
			...
			l = list.length;
			if (o.prebufferinvert)
				while (l--) o.renderStageChild(list[l]);
			else
				while (i < l) o.renderStageChild(list[i++]);
		} else {
			if (!o.inversed) {
