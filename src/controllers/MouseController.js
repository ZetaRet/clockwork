...
function MouseController() {
	...
	m.addMouseEvent = function(type) {
		var mtype = "mouseEventHandler_" + type,
			h = o[mtype] || o.mouseEventHandler;
	...
	m.stageIterator = function(child, f, customEvent, parent, list, type, childo) {
		var stage = o.stage,
			...
			mek = o.mouseEnabledKey,
			mck = o.mouseChildrenKey,
			im = o.interactiveMode,
			chi = child[interc.interactIDKey],
			chimap = chi ? interc.interactiveVars[chi] : o.defaultInteractiveMap,
			chimek = chimap[mek],
			chimck = chimap[mck];
	...
	m.mouseEventHandler = function(e, data, id, customEvent, overEvent, outEvent) {
		...
		var etype = e.type,
			stage = o.stage,
			mouseTargets = [],
			mtbi = o.mouseTargetsById,
			oldMouseTargets = id ? mtbi[id] : o.mouseTargets,
			mt, l = 0,
			i, mouseIterated = [],
			k, mdtbi = o.mouseDownTargetsById,
			mcap = o.mouseCapabilities,
			umob = o.useMouseOverBuffer,
			meet = o.mouseEnterEventType,
			mlet = o.mouseLeaveEventType,
			mmet = o.mouseMoveEventType,
			muet = o.mouseUpEventType;
		...
		var uomt = omt ? omt.unique() : null,
			luomt = uomt ? uomt.length : 0,
			umt = mt.unique(),
			lumt = umt.length,
			mto = null,
			umdt = null,
			mot = o.mouseOverTargets;
		...
		if (luomt > 0 && (etype === mlet || etype === mmet)) {
			...
			if (!umob) {
				if (uomt) {
					for (i = 0; i < luomt; i++) {
						mto = uomt[i];
						if (lumt === 0 || umt.indexOf(mto) < 0) {
							mto.dispatchEvent(outEvent, stage);
						}
					}
				}
			} else {
		...
		if (etype === muet || etype === o.mouseWheelEventType || etype === mlet) {
			...
			if (l > 0) {
				...
				for (i = 0; i < l; i++) {
					mto = umdt[i];
					if (lumt === 0 || umt.indexOf(mto) < 0) {
						mto.dispatchEvent(outsideEvent, stage);
					}
				}
		...
		if (lumt > 0 && etype === mmet) {
			...
			if (umob) {
				for (i = 0; i < lumt; i++) {
					mto = umt[i];
					if (mot.indexOf(mto) < 0) {
						mto.dispatchEvent(overEvent, stage);
						mot.push(mto);
					}
				}
			} else {
		...
		if (lumt === 0) {
			if (o.mouseCursor !== o.defaultMouseCursor) o.setMouseCursor(o.defaultMouseCursor);
			if (o.interaction.requireClean) o.interaction.clearInteractiveContext();
		}
