...
function InteractionController() {
	...
	m.updateInteractId=function(dobj, vars){
		var map=o.interactiveVars[dobj[o.interactIDKey]],k;
		if(!map || !vars)return false;
		for(k in vars)map[k]=vars[k];
		return true;
	};
	m.hitTestPoint=function(dobj,x,y,mapped,tm){
		var ctx=o.interactiveContext,ctxr=o.ctxrenderer,found=false,cid,skip=false,
			debug=o.drawDebug,debugFill=o.drawDebugFill,
			clspth=o.TYPES.CLOSE_PATH,strk=o.TYPES.STROKE,
			path=mapped?o.interactiveMap[dobj[o.interactIDKey]]:null,dbgc=o.debugChannel,dbgpx=o.debugPixel;
		...
		if(path){
			...
			var pl=path.length,il=ctxr.intlookup,i,s=ctxr.setters,c,cm;
			if(debug){
				o.requireClean=true;
	...
	m.hitTestObject=function(dobj,dobj2,mapped){
		var ctx=o.interactiveContext,ctxr=o.ctxrenderer,
		...
		if(path1&&path2){
			...
			var pl=path1.length,il=ctxr.intlookup,i,s=ctxr.setters,c,cm;
			...
			var maxsearch=ctx.canvas.height,
				...
				searchOffset=0,
				ctxdata,cdl,found=false,sp=o.useSearchPixels,spi=o.pixelSearchInt;
			if(o.useSetPixels){
