...
function ChildController() {
	...
	m.childToRoot = function(child, f, d, scope) {
		var chk = o.childrenkey,
			lk = o.listskey,
			tk = o.targetkey,
			ch, li = child[lk],
			l = li ? li.length : 0,
			lij, fr, pk = o.parentkey;
		if (!scope) scope = o;
		if (!o.useFractalRoot) {
			if (li) {
				while (lij = li[pk]) {
	...
	m.iterate = function(child, f, d, parent, list, scope, mode, skip) {
		...
		if (!skip) {
			...
			if (cht) {
				...
				if (o.bubblePhase === zetaret.global.packages.clockwork.controllers.ChildController.BUBBLEUP) {
					if (ieh) ieh.call(scope, cht);
					return o;
				} else if (o.bubblePhase === zetaret.global.packages.clockwork.controllers.ChildController.BUBBLENEXT) {
					if (ieh) ieh.call(scope, cht);
					return o;
				}
				if (!sr) {
