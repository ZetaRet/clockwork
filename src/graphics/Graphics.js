...
function Graphics() {
	...
	m.getCommand=function(type, startIndex){
		var c=o.commands,l=c.length,cc;
	...
	m.drawRadient=function(x, y, r, colors, facet, stroke, offset, circum, gdata, gradienth, pathh, alpha, ldata, edata){
		...
		var cl=colors.length,radiance=cl*facet,phase=circum/radiance,endphase,
			start=(offset||0)*Math.PI/180,xs,xe,ys,ye,i,j,radianceError,
			...
			r1=r-stroke/2,r2=r+stroke/2,useStroke=gdata&&gdata.useStroke;
	...
	m.readGraphicsData=function(cached){
		...
		for(i=0;i<cl;i++){
			...
			if(c.is(zetaret.global.packages.clockwork.graphics.GraphicData)){
				...
				fill=true;
			} else if(c.is(zetaret.global.packages.clockwork.graphics.GraphicEndFill) || c.is(zetaret.global.packages.clockwork.graphics.GraphicSolidFill) || c.is(zetaret.global.packages.clockwork.graphics.GraphicShaderFill)){
	...
	m.toIntArray=function(ctxrenderer){
		...
		var a=[],grd=o.readGraphicsData(true),l=grd.length,c,i;
		for(i=0;i<l;i++){
			c=grd[i];
			a.push(1+c[1].length);
			a.push(c[0]);
	...
	m.toDirect=function(ctxrenderer){
		...
		var a=[],grd=o.readGraphicsData(true),l=grd.length,c,il=ctxrenderer.intlookup,i;
		for(i=0;i<l;i++){
			c=grd[i];
			a.push(il[c[0]]);
			a.push(c[1]);
	...
	m.filterGraphicsData=function(types, cached){
		var f=[],grd=o.readGraphicsData(true),l=grd.length,c,i,cid,cm=o.cacheMap;
		if(cached){
			cid=types.join(',');
			if(!cm){
				cm={};
	...
	m.clearGradientCache=function(){
		var gc=o.gradientCache,gm=o.graphicsMediator,gmgc=gm?gm.gradientCache:null,gcl=gc?gc.length:0;
		if(gcl && gmgc){
			while(gcl--){
...
ProtoSS.staticProps(zetaret.global.packages.clockwork.graphics.Graphics,{EMPTY_ARRAY:[]});
